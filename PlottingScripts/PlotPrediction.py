...
#        self.n_x_bins, self.n_y_bins = 50, 50       # colormap grid dimensions for predicted position
        self.n_vx_bins, self.n_vy_bins = 20, 20# colormap grid dimensions for predicted direction
...
#                  'text.usetex': True,
    ...
    def normalize_spiketimes(self, fn=None):
        ...
        self.nspikes, self.spiketrains = utils.get_nspikes(fn, get_spiketrains=True, n_cells=self.params['n_exc'])
    ...
    def compute_pos_and_v_estimates(self, stim_idx_presented=0):
        """
        This function combines activity on the population level to estimate vx, vy
        stim_idx_presented -- if only one stimulus was simulated, but the motion parameters from a different one have been
                            presented, the stim_idx_presented is taken to indicate which motion_parameters are to be taken
        ...
        for i in xrange(self.n_bins):
            ...
            if stim_idx_presented != None:
                stim_idx = stim_idx_presented
            else:
                stim_idx = int(t / self.t_stim)
...
#            self.vy_avg[i] = np.sum(vy_pred)
            if self.params['n_grid_dimensions'] == 2:
                ...
                self.vdiff_avg[i] = np.abs(mp[2] - self.vx_avg[i])
...
#        assert (np.sum(self.theta_marginalized_binned_nonlinear) == 1), "Marginalization incorrect: %.1f" % (np.sum(self.theta_marginalized_binned_nonlinear))
    ...
    def load_spiketimes(self, cell_type):
        ...
        nspikes, spiketimes = utils.get_nspikes(fn, n_cells=n_cells, get_spiketrains=True, cell_offset=self.params['%s_offset' % cell_type])
...
#        title = '$v_x$ binned vs time'
    ...
    def plot_grid_vs_time(self, data, title='', xlabel='', ylabel='', yticks=[], \
        ...
        if time_range == None:
            ...
            x_ticklabels = ['%d' % i for i in self.t_ticks]
            x_ticks = np.linspace(0, self.n_bins, n_x_bins)
        else:
            n_time_ticks = 5
...
#        ticklabels = list(ticklabels)
    ...
    def plot_xdiff(self, fig_cnt=1, show_blank=None, time_range=None):
        ...
        if time_range == None:
            ...
            ylim = (0, 1.1 * max(self.xdiff_avg[idx0:idx1]))
            ax.set_ylim(ylim)
        ...
        output_data = np.zeros((self.t_axis.size, 2))
...
#        ax.legend()
    ...
    def plot_vx_estimates(self, fig_cnt=1, show_blank=None, time_range=None, stim_idx=0):
        if show_blank == None:
            show_blank = self.show_blank
        ax = self.fig.add_subplot(self.n_fig_y, self.n_fig_x, fig_cnt)
    ...
    def plot_vy_estimates(self, fig_cnt=1, show_blank=None, time_range=None):
        ...
        if time_range == None:
            ...
            ax.set_xlim((time_range[0], time_range[1]))
            stim_idx = int(time_range[0] / self.t_stim)
        vy = self.all_motion_params[stim_idx, 3] * np.ones(self.t_axis.size)
        ax.set_xticks(x_ticks)
        ax.set_xticklabels(x_ticklabels)
        if show_blank:
            self.plot_blank(ax, time_range)
...
#        pylab.show()
    ...
    def plot_blank(self, ax, time_range=None, c='r'):

        if time_range != None:
            stim = time_range[0] / self.t_stim
            t_start_blank = stim * self.t_stim + self.params['t_before_blank']
            t_stop_blank = t_start_blank + self.params['t_blank']

        ylim = ax.get_ylim()
        ax.plot((t_start_blank, t_start_blank), (ylim[0], ylim[1]), ls='--', c=c, lw=2)
        ax.plot((t_stop_blank, t_stop_blank), (ylim[0], ylim[1]), ls='--', c=c, lw=2)
        ax.set_ylim(ylim)
...
#def plot_prediction(stim_range=None, params=None, data_fn=None, inh_spikes = None):
    def plot_prediction(self, stim_range=None, params=None, data_fn=None, inh_spikes = None):
        """
        run the required functions to get some nice figures:
            - rasterplot for inhibitory neurons
            - rasterplot exc sorted for x, vx + prediction cmap
            - readout estimates for x, vx, and the corresponding differences
        """
        ...
        self.compute_pos_and_v_estimates()
        for i_stim, stim in enumerate(range(stim_range[0], stim_range[1])):
            ...
            self.n_fig_y = 1
            self.create_fig()
            self.plot_rasterplot(cell_type='inh_unspec', fig_cnt=1, time_range=time_range)
            ...
            self.n_fig_x = 2
            self.n_fig_y = 2
            self.create_fig()  # create an empty figure
            self.plot_input_spikes_sorted(time_range, fig_cnt=1, sort_idx=0)
            self.plot_raster_sorted(time_range, fig_cnt=1, title='Exc cells sorted by x-position', sort_idx=0)
            self.plot_x_grid_vs_time(2, time_range=time_range, stim_idx=stim)
            self.plot_input_spikes_sorted(time_range, fig_cnt=3, sort_idx=2)
            self.plot_raster_sorted(time_range, fig_cnt=3, title='Exc cells sorted by $v_x$', sort_idx=2)
            self.plot_vx_grid_vs_time(4, time_range=time_range)
            ...
            self.plot_x_estimates(1, time_range=time_range)
            self.plot_xdiff(2, time_range=time_range)
            self.plot_vx_estimates(3, time_range=time_range, stim_idx=stim)
            self.plot_vdiff(4, time_range=time_range)
...
class MetaAnalysisClass(object):
    def __init__(self, argv):
        ...
        Plotter.plot_prediction(stim_range=stim_range, params=params)
