...
def plot_contour_connectivity_tgt_adj_list(params, adj_list, tp, gid, plot_delay=False, clim=None):
    """
    adj_list is an adjacency list of the format :
    {  "tgt_gid": [ [src_gid, weight], [ , ] ], "tgt_gid_0" : [[ ...]]}
    Example for adjacency list indexed by the target:
    {
      "1545": [
          [
            100,
              5.400205183903729e-05
          ],
          [
            104,
              5.400205183903729e-05
    ...
    or it is the conn list
    """
    if not type(adj_list) == type(dict()):
        print 'This function is deprecated and is only used with adjacency lists, i.e. dictionaries indexed by either source or target.'
        print 'Will now quit'
        exit(1)
    try:
    ...
    delays = np.ones(weights.size) * params['delay_ee_global'] # TODO: could be different if it is important
