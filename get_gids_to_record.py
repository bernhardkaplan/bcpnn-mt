...
print "\nGID\tnspikes\tdistance_to_stim\tweight_to_next_cell that will see the stimulus (and weight back)\tx_pos"

for i in xrange(n-1):
    print gids[i], '\t', nspikes[gids[i]], '\t', distances[x_sorted_indices[i]], '\t\t', conn_mat[gids[i], gids[i+1]], '\t\t', \
