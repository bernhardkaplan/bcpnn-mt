...
class NetworkModel(object):
    ...
    def setup(self, load_tuning_prop=False, times={}):
        ...
        # # # # # # # # # # # # # # # # # # # # # # # # #
        self.v_init_dist = pyNN.random.RandomDistribution('normal',
...
#                        w[i] = max(self.params['w_min'], min(w[i], self.params['w_max']))
    ...
    def connect_isotropic(self, conn_type='ee'):
        ...
        w_dist = pyNN.random.RandomDistribution('normal',
                (w_mean, w_sigma),
                rng=self.rng_conn,
                constrain='redraw',
                boundaries=(0, w_mean * 10.))
        delay_dist = pyNN.random.RandomDistribution('normal',
    ...
    def connect_random(self, conn_type):
        ...
        weight_distr = pyNN.random.RandomDistribution('normal',
                (w_mean, w_sigma),
                rng=self.rng_conn,
                constrain='redraw',
                boundaries=(0, w_mean * 10.))
